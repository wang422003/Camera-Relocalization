...
def create_PoseNet_valid(weights_path=None, tune=False):
    # creates PoseNet_valid from GoogLeNet a.k.a. Inception v1 (Szegedy, 2015)
    with tf.device('/cpu:0'):
        ...
        PoseNet_valid = Model(input=input, output=[cls1_fc_pose_xyz, cls1_fc_pose_wpqr, cls2_fc_pose_xyz, cls2_fc_pose_wpqr,
                                             cls3_fc_pose_xyz, cls3_fc_pose_wpqr])

    if tune:
        if weights_path:
            weights_data = np.load(weights_path).item()
            for layer in PoseNet_valid.layers:
